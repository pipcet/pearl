# Based on the standard Term::ReadLine::Perl distribution, modified to
# work in the very limited Apple M1 Recovery Mode environment.
...
sub preinit
{
    ...
    $term_OUT = \*STDERR unless defined $term_OUT;
...
sub readline_dumb {
	...
	local $/ = "\n";
	return undef
          if !defined($line = $main::term->get_line);
	chomp($line);
	$| = $oldbar;
	select $old;
	return $line;
...
sub substr_with_props {
  ...
  unless (defined $ket) {
    warn 'bug in main, please report to its author cpan@ilyaz.org';
    $ket = '';
  }
...
print main::readline($ARGV[0], $ARGV[1]) . "\n";
1;
__END__
