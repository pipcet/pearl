...
$(BUILD)/initramfs/pearl.cpiospec: local/initramfs/pearl.cpiospec
	$(MKDIR) $(dir $@)
	@(cat $<; $(foreach file,$(patsubst $(BUILD)/initramfs/pearl/%,/%,$(wordlist 2,$(words $^),$^)),echo dir $(dir $(patsubst %/,%,$(file))) 755 0 0; echo file $(file) $(BUILD)/initramfs/pearl/$(file) 755 0 0;) (cd $(BUILD)/pearl/install/; find -type f | while read file; do echo "file $$file $(BUILD)/pearl/install/$$file 755 0 0"; done)) | sort | uniq > $@
