$(BUILD)/lvm2/done/install: $(BUILD)/lvm2/done/build
	PATH="$(CROSS_PATH):$$PATH" $(MAKE) -C $(BUILD)/lvm2/build DESTDIR=$(BUILD)/pearl/install/ install
	@touch $@
...
$(BUILD)/lvm2/done/configure: $(BUILD)/lvm2/done/copy $(BUILD)/libaio/done/install $(BUILD)/libblkid/done/install $(BUILD)/glibc/done/glibc/install $(BUILD)/gcc/done/gcc/install
	(cd $(BUILD)/lvm2/build; PATH="$(CROSS_PATH):$$PATH" ./configure --host=aarch64-linux-gnu --target=aarch64-linux-gnu --prefix=/ CFLAGS="$(CROSS_CFLAGS) -I." LDFLAGS="-L$(BUILD)/pearl/install/lib")
	@touch $@
